...
class Descriptor:
    ...
    def projection(self, projection: CRS) -> None:
        ...
        if hasattr(self, "_projection"):
            reprojecting = True
            print("Reprojecting the descriptor can be inefficient for large meshes")
        ...
        if (self.x_period is None and self.y_period is None) or reprojecting:
    ...
    def _parse_period(self, ds, dim: Literal["x", "y"]):
        ...
        if self.is_periodic and attr not in ds.attrs:
            ...
            msg = (
                f'Mesh file: "{fp}" does not have attribute `{attr}` '
                f"despite being a planar periodic mesh."
            )
    ...
    def _wrap_patches(self, patches, loc):
        ...
        if self.y_period:
            ...
            y_mask, y_sign = _find_boundary_patches(patches, loc, "y", self.y_period)
    ...
    def _compute_pole_mask(self, loc) -> ndarray:
        ...
        at_pole = np.any(np.isclose(centers.reshape(-1, 1), limits, rtol=1e-2), axis=1)
